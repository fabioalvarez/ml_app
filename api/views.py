...
def index():
    ...
    if request.method == "POST":
        ...
        if file and utils.allowed_file(file.filename):
            ...
            path_new_image = os.path.join(settings.UPLOAD_FOLDER, hashed_name)
...
def predict():
    ...
    # No file received, send response
    if "file" not in request.files:
        return rpse, 400

    # File received but no filename is provided, send response
    file = request.files["file"]
    if file.filename == "":
        return jsonify(rpse)
...
def feedback():
    ...
    if request.method == "POST":
        with open(settings.FEEDBACK_FILEPATH+".csv", 'a') as outfile:
